...
let hasContentLengthHeader = (s) => {
    String.length(s) > contentLengthHeaderLength && String.equal(String.sub(s, 0, contentLengthHeaderLength),contentLengthHeader);
};

let getContentLength = (s) => {
    let v = String.sub(s, contentLengthHeaderLength, String.length(s) - endLineLength - contentLengthHeaderLength);    
    int_of_string(v);
}
