...
describe("Preamble", ({test, describe}) => {
  test("hasContentLengthHeader", ({expect}) => {
    expect.bool(Preamble.hasContentLengthHeader("Content-Length: 100")).toBe(true);
    expect.bool(Preamble.hasContentLengthHeader("Content-Length:")).toBe(false);
  ...
  describe("parseLine", ({test, _}) => {
    ...
    test("ignores other headers for now", ({expect}) => {
        ...
        let v = Preamble.parseLine(~current=v,"A-Random-Header: some type\r\n");
        let v = Preamble.parseLine(~current=v,"\r\n");
